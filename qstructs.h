...
{
    uint8_t pubkey[32];
    uint32_t ownedtick,possessedtick,reqtick,tbd;
    int64_t extra0,extra1;
    uint64_t owned[256][2];
    uint64_t possessed[256][2];
};
...
{
    struct Asset asset;
    uint32_t tick,universeindex;
    uint8_t siblings[ASSETS_DEPTH][32];
} RespondIssuedAssets;
...
struct issuerpub
{
    const char name[8],issuer[64];    // cannot change! initialized in qhelpers.c
    uint8_t pubkey[32];
    int32_t nonzbids,bidsoffset,nonzasks,asksoffset;
    int64_t highbid,lowask;
    struct Orders_Output bids[MAXORDERBOOKSIZE],asks[MAXORDERBOOKSIZE]; // change to realloc ptr extend dynamically
};
...
{
    uint32_t currentTickOfNode,tick,txCount;
    uint8_t moneyFlew[(NUMBER_OF_TRANSACTIONS_PER_TICK+7) >> 3];
    uint8_t txDigests[][32];
};
#pragma pack(pop)


#define IPC_TICKUTC 'u'
struct ipc_tickutc_req
{
    uint8_t cmd,pad0;
    uint16_t epoch;
    uint32_t tick;
